...
function read_puzzles() {
    ...
    res = res.filter(_ => _.tags.includes('rookEndgame'))
    ...
    fs.writeFileSync('rook-u1600.pgn', u1600.map(pz_pgn).join('\n'))
    fs.writeFileSync('rook-u2000.pgn', u2000.map(pz_pgn).join('\n'))
    fs.writeFileSync('rook-u2200.pgn', u2200.map(pz_pgn).join('\n'))
    fs.writeFileSync('rook-mix200.pgn', mix200.map(pz_pgn).join('\n'))
}
